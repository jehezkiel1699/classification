...
$start = ($page - 1) * $limit;
$sql = "SELECT * FROM news WHERE kategori='tekno' ORDER BY str_to_date(`tanggal`, '%d/%m/%Y') DESC LIMIT $start, $limit";
...
$sql = "SELECT count(id) AS id FROM news WHERE kategori='tekno'";
$res = mysqli_query($con, $sql);
$newsCount = array();
while ($row = mysqli_fetch_assoc($res)) {
	$newsCount[] = $row;
							...
							<div class="col-md-12">
								<nav aria-label="Page navigation example">
									<ul class="pagination justify-content-center">
										<?php if($start == 0){ ?>
										<li class="page-item">
											<a class="page-link" href="teknologi.php?page=<?= $Previous; ?>" tabindex="-1" style="display:none;">Previous</a>
										</li>
										<?php }else{ ?>
										<li class="page-item">
											<a class="page-link" href="teknologi.php?page=<?= $Previous; ?>" tabindex="-1">Previous</a>
										</li>
										<?php } ?>
										<!--<?php if($page==0){ ?>
										<li class="page-item">
										...
										<!-- <?php for($i = 1; $i<= $pages; $i++) :?>
											<li><a class="page-link" href="teknologi.php?page=<?= $i; ?>" ><?= $i; ?></a></li>
										<?php endfor; ?> -->
										...
										<?php if($page != $pages){ ?>
										<li class="page-item">
									    	<a class="page-link" href="teknologi.php?page=<?= $Next; ?>">Next</a>
									    </li>
									    <?php } ?>
									</ul>
								</nav>
